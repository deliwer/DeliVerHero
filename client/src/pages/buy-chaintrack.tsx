...
export default function BuyChaintrackPage() {
  ...
  const [tab, setTab] = useState<"marketplace" | "reverse_bids" | "orders" | "account" | "founder_admin">("marketplace");
  const [founderPwd, setFounderPwd] = useState("");
  const [founderAuthed, setFounderAuthed] = useState(() => sessionStorage.getItem("buy_founder_auth") === "1");
  const [founderPwdError, setFounderPwdError] = useState("");
  const FOUNDER_PWD = "deliwer-admin-2026";
  const { toast } = useToast();
    ...
    <div className="min-h-screen bg-[#080c14] text-white">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex gap-1 mb-6 bg-slate-900 border border-slate-800 rounded-xl p-1">
          ...
          <button
            onClick={() => setTab("founder_admin")}
            className={`flex items-center justify-center gap-2 py-2.5 px-3 rounded-lg text-sm font-medium transition-all
              ${tab === "founder_admin" ? "bg-rose-700 text-white" : "text-slate-600 hover:text-slate-400"}`}
            title="Founder Admin"
            data-testid="tab-founder_admin"
          >
        ...
        {tab === "account" && (
          <div className="max-w-2xl space-y-5">
              ...
              <div className="bg-amber-900/20 border border-amber-700/30 rounded-xl p-5">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <a
                      href="https://wa.me/971523906019?text=ChainTrack%20Buy%20KYC%20Verification"
                      target="_blank"
            ...
            <div className="flex gap-3">
              <a
                href="https://wa.me/971523906019?text=ChainTrack%20Buy%20Support"
                target="_blank"
                rel="noopener noreferrer"
            ...
            {!founderAuthed ? (
              <div className="flex items-center justify-center py-20">
                <div className="w-full max-w-sm">
                  <div className="text-center mb-6">
                    <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-rose-500/15 border border-rose-500/30 mb-3">
                      <Lock className="w-6 h-6 text-rose-400" />
                    </div>
                    <h2 className="text-white font-black text-xl">Founder Admin</h2>
                    <p className="text-slate-400 text-sm mt-1">Unified supplier management</p>
                  </div>
                  <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                    <div className="space-y-4">
                      <div>
                        <Label className="text-slate-300 text-xs mb-1 block uppercase tracking-widest">Password</Label>
                        <div className="relative">
                          <Input
                            type="password"
                            value={founderPwd}
                            onChange={e => { setFounderPwd(e.target.value); setFounderPwdError(""); }}
                            onKeyDown={e => {
                              if (e.key === "Enter") {
                                if (founderPwd === FOUNDER_PWD) {
                                  sessionStorage.setItem("buy_founder_auth", "1");
                                  setFounderAuthed(true);
                                } else {
                                  setFounderPwdError("Incorrect password");
                                  setFounderPwd("");
                                }
                              }
                            }}
                            ...
                            autoFocus
                            data-testid="input-founder-admin-password"
                          />
                        </div>
                        {founderPwdError && <p className="text-red-400 text-xs mt-1">{founderPwdError}</p>}
                      </div>
                      <Button
                        onClick={() => {
                          if (founderPwd === FOUNDER_PWD) {
                        ...
                        data-testid="button-founder-admin-login"
                      >
                        <Lock className="w-4 h-4 mr-2" /> Unlock Admin Panel
                      </Button>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-8">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs font-bold text-rose-400 uppercase tracking-widest">Founder Admin</span>
                    </div>
                    <h2 className="text-white font-black text-2xl">Unified Supplier Dashboard</h2>
                    <p className="text-slate-400 text-sm mt-0.5">All supplier feeds consolidated · {lots.length} total lots</p>
                  </div>
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => { sessionStorage.removeItem("buy_founder_auth"); setFounderAuthed(false); setTab("marketplace"); }}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {[
                    { label: "Total Lots", value: lots.length, color: "text-orange-400", bg: "bg-orange-500/10 border-orange-500/20" },
                    { label: "Supplier Feeds", value: feedLots.length, color: "text-emerald-400", bg: "bg-emerald-500/10 border-emerald-500/20" },
                    { label: "Reverse Bid", value: auctionLots.length, color: "text-purple-400", bg: "bg-purple-500/10 border-purple-500/20" },
                    { label: "Export Ready", value: lots.filter(l => l.exportReady).length, color: "text-sky-400", bg: "bg-sky-500/10 border-sky-500/20" },
                  ].map(s => (
                ...
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h3 className="text-white font-bold">ChainTrack Lots</h3>
                      <p className="text-slate-400 text-xs">buy.chaintrack.com · {lots.length} lots active</p>
                    </div>
                    <span className="ml-auto text-xs bg-orange-500/15 text-orange-400 border border-orange-500/25 px-2 py-0.5 rounded-full font-semibold">LIVE</span>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-slate-800 text-xs text-slate-400 uppercase tracking-wider">
                          ...
                          <th className="text-left pb-2 pr-4">Grade</th>
                      ...
                      <tbody>
                        {lots.slice(0, 10).map(lot => (
                          <tr key={lot.id} className="border-b border-slate-800/60 hover:bg-slate-900/40 transition-colors">
                            <td className="py-2.5 pr-4 font-mono text-xs text-slate-400">{lot.lotNumber}</td>
                            <td className="py-2.5 pr-4 text-white font-medium max-w-[180px] truncate">{lot.productName}</td>
                            <td className="py-2.5 pr-4 text-slate-400 text-xs">{lot.supplierName}</td>
                            <td className="py-2.5 pr-4"><GradeBadge grade={lot.grade} /></td>
                            <td className="py-2.5 pr-4 text-right text-white">{lot.availableQty.toLocaleString()}</td>
                            <td className="py-2.5 pr-4 text-right text-orange-400 font-bold">{formatUSD(lot.unitPrice)}</td>
                            <td className="py-2.5">
                              <span className={`text-xs ${lot.status === "active" ? "text-emerald-400" : "text-slate-500"}`}>
                                {lot.status}
                              </span>
                            </td>
                          </tr>
                    ...
                    {lots.length > 10 && (
                      <p className="text-xs text-slate-500 text-center mt-3">+{lots.length - 10} more lots — switch to Supplier Feeds tab to view all</p>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h3 className="text-white font-bold">WeSellCellular (WSC)</h3>
                      <p className="text-slate-400 text-xs">buy.wesellcellular.com · US carrier-graded refurbs</p>
                    </div>
                    <a
                      href="/buy/wsc"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="ml-auto text-xs bg-sky-500/15 text-sky-400 border border-sky-500/25 px-3 py-1 rounded-full font-semibold hover:bg-sky-500/25 transition-colors flex items-center gap-1"
                    >
                  ...
                  <div className="grid grid-cols-3 gap-3 text-center mb-4">
                    {[
                      { label: "Supplier", value: "WeSellCellular LLC" },
                      { label: "Origin", value: "USA" },
                      { label: "Specialty", value: "Carrier-Graded iPhones" },
                    ].map(i => (
                      <div key={i.label} className="bg-slate-800 rounded-lg p-3">
                        <p className="text-white text-sm font-semibold">{i.value}</p>
                        <p className="text-slate-500 text-xs mt-0.5">{i.label}</p>
                      </div>
                    ))}
                  </div>
                  <a
                    href={`https://wa.me/971523906019?text=WSC+Admin+%E2%80%94+Supplier+Feed+Review`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 text-xs bg-green-700/20 border border-green-700/30 text-green-400 hover:bg-green-700/30 rounded-lg px-3 py-1.5 transition-colors"
                  >
                    <SiWhatsapp className="w-3.5 h-3.5" /> Contact WSC Team
                  </a>
                </div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h3 className="text-white font-bold">KT Corp Worldwide</h3>
                      <p className="text-slate-400 text-xs">buy.ktcorpworldwide.com · Auction lots from Korea & Asia</p>
                    </div>
                    <a
                      href="/buy/ktcorp"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="ml-auto text-xs bg-amber-500/15 text-amber-400 border border-amber-500/25 px-3 py-1 rounded-full font-semibold hover:bg-amber-500/25 transition-colors flex items-center gap-1"
                    >
                      <ExternalLink className="w-3 h-3" /> Open Portal
                    </a>
                  </div>
                  <div className="grid grid-cols-3 gap-3 text-center mb-4">
                    {[
                      { label: "Supplier", value: "KT Corp Worldwide" },
                      { label: "Origin", value: "Korea / Asia" },
                      { label: "Specialty", value: "Auction Electronics" },
                    ].map(i => (
                      <div key={i.label} className="bg-slate-800 rounded-lg p-3">
                        <p className="text-white text-sm font-semibold">{i.value}</p>
                        <p className="text-slate-500 text-xs mt-0.5">{i.label}</p>
                  ...
                  <a
                    href={`https://wa.me/971523906019?text=KT+Corp+Admin+%E2%80%94+Supplier+Feed+Review`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 text-xs bg-green-700/20 border border-green-700/30 text-green-400 hover:bg-green-700/30 rounded-lg px-3 py-1.5 transition-colors"
                  >
                    <SiWhatsapp className="w-3.5 h-3.5" /> Contact KT Corp Team
                  </a>
                </div>

                {/* Quick Actions */}
                <div className="bg-rose-950/20 border border-rose-700/30 rounded-xl p-5">
                  <h4 className="text-rose-300 font-bold text-sm mb-3 flex items-center gap-2">
                    <Zap className="w-4 h-4" /> Quick Admin Actions
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <button onClick={() => refetchLots()} className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs px-3 py-2.5 rounded-lg transition-colors">
                      <RefreshCw className="w-3.5 h-3.5" /> Refresh All Lots
                    </button>
                    <a href="/buy/wsc" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-sky-400 text-xs px-3 py-2.5 rounded-lg transition-colors">
                      <ExternalLink className="w-3.5 h-3.5" /> WSC Admin View
                    </a>
                    <a href="/buy/ktcorp" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-amber-400 text-xs px-3 py-2.5 rounded-lg transition-colors">
                      <ExternalLink className="w-3.5 h-3.5" /> KT Corp Admin View
                    </a>
                  </div>
                </div>
              </div>
