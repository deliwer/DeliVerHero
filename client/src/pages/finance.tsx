          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-4">
                ...
                <div className="text-lg font-bold text-white mt-1" data-testid="text-calc-rent-cost">{fmtAED(result.rentTotal)}</div>
                <div className="text-[11px] text-slate-500 mt-0.5">With 5% annual escalation</div>
              </div>
            </div>

            <div className={`rounded-xl border p-5 ${result.savings > 0 ? "border-emerald-500/40 bg-emerald-500/10" : "border-amber-500/40 bg-amber-500/10"}`}>
              <div className={`text-[11px] font-bold uppercase tracking-widest ${result.savings > 0 ? "text-emerald-300" : "text-amber-300"}`}>
                {result.savings > 0 ? "Buying saves you (net equity)" : "Renting costs less by"}
              </div>
              <div className={`text-2xl font-black mt-1 ${result.savings > 0 ? "text-emerald-300" : "text-amber-300"}`} data-testid="text-calc-savings">
                {fmtAED(Math.abs(result.savings))}
              </div>
              <div className="text-[11px] text-slate-400 mt-1">Over your {horizon}-year horizon</div>
            </div>

            <button
              onClick={() => window.open(`https://wa.me/${WA_NUMBER}?text=${encodeURIComponent(
              ...
              className="w-full rounded-xl border border-emerald-500/30 bg-emerald-500/10 hover:bg-emerald-500/20 transition p-4 text-left"
              data-testid="button-calc-wa"
            >
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-sm font-bold text-emerald-300">Get personalised eligibility report</div>
                  <div className="text-xs text-slate-400 mt-0.5">Based on these numbers — WhatsApp reply in minutes</div>
                </div>
...
export default function FinancePage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <Helmet>
        <title>Dubai Home Finance & Mortgage Solutions | DeliWer × Yubi</title>
        <meta
          name="description"
          content="Compare mortgages, discover developer payment plans, and get financing approvals in Dubai. Powered by Yubi credit solutions — pre-approval from top UAE banks in 48 hours."
        />
        <meta
          name="keywords"
          content="Dubai mortgage, UAE home loan, Dubai property finance, Yubi credit, developer payment plan Dubai, home financing Dubai"
        />
        <meta property="og:title" content="Dubai Home Finance & Mortgage | DeliWer × Yubi" />
        <meta
          property="og:description"
          content="Compare mortgages from 8+ UAE banks, explore developer payment plans and get pre-approved — with Yubi credit solutions powering the process."
        />
      </Helmet>

      {/* ── Yubi Partnership Banner ─────────────────────────────────── */}
      <div className="bg-gradient-to-r from-violet-950 via-slate-900 to-violet-950 border-b border-violet-500/20">
        <div className="max-w-5xl mx-auto px-4 py-3 flex flex-col sm:flex-row items-center justify-between gap-2 text-sm">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-slate-300">Mortgage &amp; credit solutions powered by</span>
            <span className="font-black text-violet-300 tracking-tight">Yubi</span>
            <Badge className="text-[10px] bg-violet-500/15 text-violet-300 border-violet-500/30 hidden sm:inline-flex">Credit Marketplace</Badge>
          </div>
          <a
            href="https://yubi.com"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 text-[11px] font-bold text-violet-300 hover:text-violet-200 transition-colors"
            data-testid="link-yubi"
          >
            Visit yubi.com <ExternalLink className="w-3 h-3" />
          </a>
        </div>
      </div>
      ...
      {/* ── Yubi Credit Solutions Section ────────────────────────────── */}
      <section className="relative scroll-mt-24 border-b border-slate-900 bg-gradient-to-br from-slate-950 via-violet-950/20 to-slate-950">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-14 lg:py-16">
          <div className="text-center max-w-3xl mx-auto mb-10">
            <Badge className="bg-violet-500/15 text-violet-300 border-violet-500/30 mb-3">
              <PiggyBank className="w-3.5 h-3.5 mr-1.5" /> Powered by Yubi · Credit Marketplace
            </Badge>
            <h2 className="text-3xl sm:text-4xl font-bold mb-3" data-testid="heading-yubi">
              Smarter credit decisions with Yubi
            </h2>
            <p className="text-slate-300">
              Yubi is the Middle East&apos;s leading credit marketplace — connecting borrowers with institutional lenders, NBFCs and banks to unlock the best mortgage and personal credit terms, faster.
            </p>
          </div>
            ...
            {[
              {
                ...
                desc: "Access 100+ lenders on a single platform. Get competing offers and choose the best rate — no branch visits.",
                color: "violet",
              },
              {
                icon: ShieldCheck,
                title: "Credit Score Optimisation",
                desc: "Yubi analyses your Al Etihad credit profile and guides you to improve eligibility before you apply — increasing approval odds.",
                color: "emerald",
              },
              {
                icon: Percent,
                title: "Personalised Rate Matching",
                desc: "AI-matched loan products based on your income, residency and property type — fixed, variable and Islamic options.",
                color: "cyan",
              },
            ].map(({ icon: Icon, title, desc, color }) => (
              <div
                key={title}
                className={`rounded-xl border border-${color}-500/30 bg-${color}-500/5 p-6`}
                data-testid={`card-yubi-${title.toLowerCase().replace(/\s+/g, "-")}`}
              >
                <div className={`w-10 h-10 rounded-xl bg-${color}-500/15 border border-${color}-500/30 flex items-center justify-center mb-4`}>
                  <Icon className={`w-5 h-5 text-${color}-300`} />
                </div>
                <h3 className={`font-bold text-${color}-200 mb-2`}>{title}</h3>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <a href="https://yubi.com" target="_blank" rel="noopener noreferrer" data-testid="link-yubi-cta">
              <Button className="h-11 bg-violet-600 hover:bg-violet-500 text-white font-black gap-2">
                <ExternalLink className="w-4 h-4" /> Explore Yubi Credit Solutions
              </Button>
            </a>
            <Button
              variant="outline"
              className="h-11 border-violet-500/40 text-violet-300 hover:bg-violet-500/10"
              onClick={() => window.open(`https://wa.me/${WA_NUMBER}?text=${encodeURIComponent("Hi DeliWer — I'd like to explore Yubi credit solutions for my home purchase in Dubai.")}`, "_blank")}
              data-testid="button-yubi-wa"
            >
              <MessageCircle className="w-4 h-4 mr-2" /> Ask us about Yubi
            </Button>
          </div>
        </div>
      </section>
      ...
      {/* ── SECTION 4: MOVE-IN CONCIERGE TEASER ─────────────────────── */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          ...
          {showFullDisclosure && (
            <div className="grid lg:grid-cols-2 gap-5">
              <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6" data-testid="block-disclosure-nature">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <div className="text-sm font-black uppercase tracking-widest text-emerald-300">Nature of service</div>
                </div>
                <ul className="space-y-2 text-sm text-slate-300">
                  ...
                  <li className="flex gap-2"><CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" /><span>All <span className="text-white font-semibold">property transactions</span> are executed by RERA-registered brokers and the <span className="text-white font-semibold">Dubai Land Department (DLD)</span>.</span></li>
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6" data-testid="block-disclosure-no-advice">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <div className="text-sm font-black uppercase tracking-widest text-amber-300">No advice · Indicative figures</div>
                </div>
                <ul className="space-y-2 text-sm text-slate-300">
                  <li className="flex gap-2"><CheckCircle2 className="w-4 h-4 text-amber-300 shrink-0 mt-0.5" /><span>Content on this page is <span className="text-white font-semibold">general information only</span> and does not constitute legal, tax, investment or financial advice.</span></li>
                  <li className="flex gap-2"><CheckCircle2 className="w-4 h-4 text-amber-300 shrink-0 mt-0.5" /><span>Rates, LTV, tenure, fees and yields shown are <span className="text-white font-semibold">indicative</span>, may change without notice and are subject to lender / developer approval.</span></li>
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6" data-testid="block-disclosure-liability">
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-8 h-8 rounded-lg bg-rose-500/15 border border-rose-500/30 text-rose-300 flex items-center justify-center">
                    <ShieldCheck className="w-4 h-4" />
                  </div>
                  <div className="text-sm font-black uppercase tracking-widest text-rose-300">Liability protection</div>
                </div>
                <ul className="space-y-2 text-sm text-slate-300">
                  <li className="flex gap-2"><CheckCircle2 className="w-4 h-4 text-rose-300 shrink-0 mt-0.5" /><span>DeliWer accepts <span className="text-white font-semibold">no liability</span> for losses, damages, missed opportunities or any direct, indirect, incidental or consequential claim.</span></li>
                  <li className="flex gap-2"><CheckCircle2 className="w-4 h-4 text-rose-300 shrink-0 mt-0.5" /><span>All payments — down payments, fees, instalments, transfer charges — are made <span className="text-white font-semibold">directly to the bank, developer or DLD</span>. DeliWer never holds client funds.</span></li>
                  <li className="flex gap-2"><CheckCircle2 className="w-4 h-4 text-rose-300 shrink-0 mt-0.5" /><span>DeliWer is compensated by referral fees from finance / property partners — at <span className="text-white font-semibold">no extra cost</span> to the buyer.</span></li>
                </ul>
              </div>

              <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6" data-testid="block-disclosure-data">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <div className="text-sm font-black uppercase tracking-widest text-cyan-300">WhatsApp · Data &amp; consent</div>
                </div>
                <ul className="space-y-2 text-sm text-slate-300">
                  <li className="flex gap-2"><CheckCircle2 className="w-4 h-4 text-cyan-300 shrink-0 mt-0.5" /><span>By initiating a WhatsApp chat you consent to DeliWer contacting you on that number.</span></li>
                  <li className="flex gap-2"><CheckCircle2 className="w-4 h-4 text-cyan-300 shrink-0 mt-0.5" /><span>Personal data is processed in line with the <span className="text-white font-semibold">UAE Personal Data Protection Law (PDPL)</span>.</span></li>
                  <li className="flex gap-2"><CheckCircle2 className="w-4 h-4 text-cyan-300 shrink-0 mt-0.5" /><span>You may withdraw consent at any time by sending <span className="text-white font-semibold">"STOP"</span> on the same WhatsApp thread.</span></li>
                </ul>
              </div>
            </div>
          )}
