import { Link } from "wouter";

export const BROKER_WA_LINK =
  "https://wa.me/971523906019?text=Hi%20DeliWer%20%E2%80%94%20I%E2%80%99m%20a%20RERA%20broker.%20Interested%20in%20your%20commission-only%20rental%20%2B%20distress-sale%20referral%20network%20across%20all%20major%20Dubai%20developers.";

export function BrokerAccessBanner({ compact = false }: { compact?: boolean }) {
  return (
    <div
      className="bg-gradient-to-r from-amber-500/15 via-emerald-500/10 to-amber-500/15 border-y border-amber-500/30"
      data-testid="banner-broker-access"
    >
      <div className="max-w-6xl mx-auto px-4 py-3 flex flex-wrap items-center gap-3 justify-between">
        <div className="flex items-center gap-3 min-w-0">
          <span className="text-[10px] font-black uppercase tracking-widest text-amber-300 bg-amber-500/20 border border-amber-500/40 px-2 py-0.5 rounded-full whitespace-nowrap">
            🔥 Broker Network
          </span>
          {!compact && (
            <p className="text-xs sm:text-sm text-slate-200 truncate">
              <strong className="text-white">Rental-first</strong> + distress sale inventory across{" "}
              <strong className="text-white">Emaar · DAMAC · Nakheel · Sobha · Aldar</strong> —{" "}
              <strong className="text-emerald-300">100% commission-only</strong>, zero signup.
            </p>
          )}
          {compact && (
            <p className="text-xs text-slate-200 truncate">
              <strong className="text-white">Rental + distress</strong> · all major developers ·{" "}
              <strong className="text-emerald-300">commission-only</strong>
            </p>
          )}
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <Link
            href="/realestate#request-access"
            className="text-xs px-3 py-1.5 bg-amber-500 hover:bg-amber-400 text-black font-bold rounded-lg transition-all whitespace-nowrap"
            data-testid="banner-button-request-access"
          >
            Request Access →
          </Link>
          <a
            href={BROKER_WA_LINK}
            target="_blank"
            rel="noopener noreferrer"
            className="text-xs px-3 py-1.5 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-200 border border-emerald-500/40 font-bold rounded-lg transition-all whitespace-nowrap"
            data-testid="banner-button-whatsapp"
          >
            💬 WhatsApp
          </a>
        </div>
      </div>
    </div>
  );
}

export function StickyBrokerWhatsApp() {
  return (
    <a
      href={BROKER_WA_LINK}
      target="_blank"
      rel="noopener noreferrer"
      data-testid="sticky-broker-whatsapp"
      className="fixed bottom-5 right-5 z-50 inline-flex items-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-semibold px-4 py-3 rounded-full shadow-lg shadow-emerald-500/30 transition-transform hover:scale-105"
      aria-label="Chat broker desk on WhatsApp"
    >
      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
      </svg>
      <span className="hidden sm:inline">Broker WhatsApp</span>
    </a>
  );
}

export function BrokerEnhancementWrapper({
  children,
  compactBanner = false,
}: {
  children: React.ReactNode;
  compactBanner?: boolean;
}) {
  return (
    <>
      <BrokerAccessBanner compact={compactBanner} />
      {children}
      <StickyBrokerWhatsApp />
    </>
  );
}
