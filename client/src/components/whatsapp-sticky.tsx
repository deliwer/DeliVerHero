...
export function WhatsAppSticky() {
  ...
  const WAButton = (
    <button
      ...
      title="Chat on WhatsApp — reply within 10 min"
      className="inline-flex items-center gap-2 bg-[#25D366] hover:bg-[#1ebe5b] text-white font-semibold px-4 py-3 rounded-full shadow-lg shadow-emerald-900/30 transition-transform hover:scale-105 active:scale-95"
    >
      <WhatsAppIcon />
  ...
  const TGButton = (
    <button
      ...
      data-testid="button-telegram-sticky"
      title="Message us on Telegram — works when WA is down"
      className="inline-flex items-center gap-2 bg-[#229ED9] hover:bg-[#1a8bbf] text-white font-semibold px-4 py-3 rounded-full shadow-lg shadow-sky-900/30 transition-transform hover:scale-105 active:scale-95"
    >
      <TelegramIcon />
      <span className="hidden sm:inline whitespace-nowrap">Telegram</span>
    </button>
  );
  ...
  return (
    <div className="fixed bottom-5 right-5 z-[9999] flex flex-col items-end gap-2">
      {/*
        ...
        Both always visible; hierarchy shifts to reduce missed contacts.
      */}
      {isMobile ? (
        <>
          {WAButton}
